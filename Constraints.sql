...
# NOT NULL 
# NOT NULL 제약조건이 지정되어 있는 컬럼에는 삽입 및 수정 작업에서 필수로 데이터를 입력해야 함
...
# UNIQUE
# UNIQUE 제약조건이 지정되어 있는 컬럼에는 각 레코드에서 해당하는 컬럼 값을 중복하여 삽입 혹은 수정할 수 없음
...
# PRIMARY KEY 제약 조건이 지정되어 있는 컬럼은 NOT NULL과 UNIQUE 제약조건을 포함하고 있음
...
CREATE TABLE primary_table_2(
	primary_column INT,
    column1 INT,
    CONSTRAINT primary_table_2_pk PRIMARY KEY (primary_column)
);
...
# DEFAULT 
# DEFAULT 제약조건이 지정되어 있는 컬럼에 삽입 작업시에 값을 지정하지 않으면 (null이 지정될 때) 삽입될 기본 값을 명시함
...
# 1. 만약 테이블을 삭제하려고 할 때 만약 해당 테이블을 참조하고 있는 테이블이 있다면 삭제 할 수 없음
# 2. 특정 컬럼에 참조제약조건을 지정하려 한다면 참조하는 컬럼에 데이터 타입과 일치해야 함 
...
CREATE TABLE sub_table (
	column_1 INT,
    column_2 INT,
    CONSTRAINT sub_table_fk FOREIGN KEY (column_2) 
    REFERENCES super_table (primary_column)
);

SELECT * FROM super_table;

# FOREIGN KEY 제약조건이 지정되어 있는 컬럼의 값은 참조하고있는 테이블의 컬럼에 있는 데이터만 지정할 수 있음
...
CREATE TABLE sub_table_2 (
	column_1 INT,
    column_2 INT,
    CONSTRAINT sub_table_2_fk FOREIGN KEY (column_2) 
    REFERENCES super_table (primary_column)
    ON UPDATE CASCADE 
    ON DELETE SET NULL
);
