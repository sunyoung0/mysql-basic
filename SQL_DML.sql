...
# WHERE 문이 지정되지 않으면 모든 데이터에 대해서 작업이 이루어짐
...
# DELETE 
# 테이블에서 특정 혹은 전체 레코드를 삭제하고자 할 때 사용하는 명령어
...
# 명령어 뒤에 스키마의 명이 적히면 DDL, FROM이 적히면 DML
...
# WHERE
# DML 중 검색, 수정, 삭제에 대해서 특정한 조건에 부합하는 레코드에 대해서만 작업이 이루어지도록 하는 연산자

# WHERE 조건을 잘 적는 것 만으로도 대부분의 작업을 수원하게 할 수 있다.
# WHERE 조건에는 비교 연산자 / 논리 연산자가 들어옴
...
# NOT IN() : 인자로 전달된 리스트의 값이 존재하지 않으면 true, 아니면 false
...
# 해체제거업자사업체명이 '우성건설'인 데이터의 '건물명_설비명' , '위치_소재지'를 검색
...
SELECT * 
FROM namgu 
WHERE 해체제거업자사업체명 = '성수개발' OR 해체제거업자사업체명 = '우성건설' OR 해체제거업자사업체명 = '고은산업';
...
# '(%,_)특정패턴문자(%,_)'
# 1. '%용호%' : 앞 뒤 어떠한 문자가 오든 '용호'라는 단어가 포함되어 있으면 true
# 2. '%용호' : 앞에 어떠한 문자가 오든 '용호'라는 단어로 끝나면 true
# 3. '용호%' : 뒤에 어떠한 문자가 오든 '용호'라는 단어로 시작하면 true
# 4. '_용호_' : 앞 뒤에 한 문자가 오고 '용호'라는 단어를 포함하고 길이가 4면 true
...
# ORDER BY 문
# 특정 컬럼을 기준으로 정렬된 결과를 보고자 할 때 사용
# ASC : 오름차순 정렬
# DESC : 내림차순 정렬 

# ORDER BY column1 (ASC / DESC), ...
# ASC는 생략 가능하고 만약 여러개의 컬럼을 ORDER BY문에 지정했다면 먼저 작성한 컬럼순으로 정렬
